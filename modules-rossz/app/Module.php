    ...
    public function init(): void
    {
        ...
        $twig->addFilter(new TwigFilter('linkTargets', function (?string $html): string {
            ...
            $result = preg_replace_callback($pattern, function ($m) use ($currentHost) {
                ...
                // Skip belső/különleges linkek
                if ($href === '' || str_starts_with($href, '#') || str_starts_with($href, 'mailto:') || str_starts_with($href, 'tel:')) {
                    return $fullTag;
                }

                if (!preg_match('#^https?://#i', $href)) {
                    return $fullTag; // relatív → békén hagyjuk
                }

                $host = parse_url($href, PHP_URL_HOST);
                if ($currentHost && $host && strcasecmp($host, $currentHost) === 0) {
                    return $fullTag; // belső → békén hagyjuk
                }

                // target, rel hozzáadása
                ...
                if (preg_match('/\brel\s*=\s*"([^"]*)"/i', $fullTag, $rm)) {
                    ...
                    $fullTag = preg_replace('/\brel\s*=\s*"([^"]*)"/i', 'rel="'.implode(' ', $rels).'"', $fullTag, 1);
                } else {
                    $fullTag = preg_replace('/>$/', ' rel="noopener noreferrer">', $fullTag, 1);
                }
